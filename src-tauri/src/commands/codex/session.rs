...
pub struct CodexExecutionOptions {
    ...
    /// Tab ID for multi-window session isolation
    /// Used to filter global events and prevent cross-tab message leakage
...
pub async fn execute_codex(
    ...
    execute_codex_process(session_id, cmd, prompt, options.project_path.clone(), options.tab_id.clone(), app_handle).await
}
...
/// 🔒 CRITICAL FIX: 添加 tab_id 参数，用于全局事件中标识消息来源，解决多窗口并发时的消息串扰
async fn execute_codex_process(
    ...
    // 🔒 CRITICAL FIX: 包含 tab_id 用于前端过滤，解决多窗口并发时的消息串扰
    ...
    log::info!("Codex session initialized with ID: {}, tab_id: {:?}", session_id, tab_id);
    ...
    let mut child = match cmd.spawn() {
        Ok(child) => child,
        Err(e) => {
            emit_codex_error(&app_handle, &session_id, "启动 Codex 失败", Some(&e.to_string()), tab_id.as_ref());
    ...
    let stdout = match child.stdout.take() {
        Some(stdout) => stdout,
        None => {
            emit_codex_error(&app_handle, &session_id, "启动 Codex 失败：无法捕获 stdout", None, tab_id.as_ref());
            let _ = child.kill().await;
            return Ok(());
        }
    };
    let stderr = match child.stderr.take() {
        Some(stderr) => stderr,
        None => {
            emit_codex_error(&app_handle, &session_id, "启动 Codex 失败：无法捕获 stderr", None, tab_id.as_ref());
            let _ = child.kill().await;
            return Ok(());
        }
    };
    ...
    tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = reader.next_line().await {
            if !line.trim().is_empty() {
                ...
                // 🔒 CRITICAL FIX: 全局事件包含 tab_id，用于前端过滤多窗口消息
...
/// 🔒 CRITICAL FIX: 添加 tab_id 参数用于全局事件隔离
fn emit_codex_error(app_handle: &AppHandle, session_id: &str, message: &str, detail: Option<&str>, tab_id: Option<&String>) {
