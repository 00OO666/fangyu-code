...
import { Trash2, RotateCcw, FolderOpen, AlertTriangle, Archive, CheckCircle2 } from "lucide-react";
    ...
    try {
      ...
      for (const projectId of hiddenIds) {
        ...
        if (projectId.includes("-") && !projectId.includes("--")) {
          decodedPath = projectId.replace(/-/g, "/").replace(/^C\//, "C:/").replace(/^\/+/, "/");
        }
  ...
  if (deletedProjects.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-muted-foreground">{t("deletedProjects.deletedProjectsInfo")}</p>
      </div>
    );
  }
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <Alert>
        ...
        <AlertDescription>{t("deletedProjects.description")}</AlertDescription>
      </Alert>
            ...
            <div className="p-4 rounded-xl bg-card border border-border/40 shadow-sm hover:shadow-md transition-all duration-300">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 ml-4">
                  <Badge
                    variant="secondary"
                    className="shrink-0 bg-muted/50 text-muted-foreground hover:bg-muted/50"
                  >
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t("deletedProjects.permanentDeleteWarning")}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              variant="outline"
