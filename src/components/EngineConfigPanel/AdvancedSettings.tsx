...
import { ChevronDown, ChevronRight, Settings2, Terminal, Eye, EyeOff, Info } from 'lucide-react';
...
const CLAUDE_ENV_VARS: {
    ...
    description: string;
        ...
        { key: 'ANTHROPIC_API_KEY', label: 'ANTHROPIC_API_KEY', type: 'secret', placeholder: 'sk-ant-...', description: 'Anthropic API 密钥' },
        { key: 'ANTHROPIC_BASE_URL', label: 'ANTHROPIC_BASE_URL', type: 'text', placeholder: 'https://api.anthropic.com', description: 'API 端点 URL' },
        { key: 'ANTHROPIC_AUTH_TOKEN', label: 'ANTHROPIC_AUTH_TOKEN', type: 'secret', placeholder: 'Bearer token', description: '认证令牌（部分代理商需要）' },
        { key: 'ANTHROPIC_MODEL', label: 'ANTHROPIC_MODEL', type: 'text', placeholder: 'claude-sonnet-4-20250514', description: '默认使用的模型' },
        { key: 'API_TIMEOUT_MS', label: 'API_TIMEOUT_MS', type: 'number', placeholder: '60000', description: 'API 请求超时时间（毫秒）' },
        { key: 'MAX_THINKING_TOKENS', label: 'MAX_THINKING_TOKENS', type: 'number', placeholder: '10000', description: '最大思考 Token 数' },
        { key: 'CLAUDE_CODE_MAX_OUTPUT_TOKENS', label: 'CLAUDE_CODE_MAX_OUTPUT_TOKENS', type: 'number', placeholder: '16000', description: '最大输出 Token 数' },
        { key: 'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC', label: 'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC', type: 'boolean', description: '禁用非必要网络流量' },
        { key: 'CLAUDE_CODE_DISABLE_TELEMETRY', label: 'CLAUDE_CODE_DISABLE_TELEMETRY', type: 'boolean', description: '禁用遥测数据收集' },
        { key: 'CLAUDE_CODE_USE_BEDROCK', label: 'CLAUDE_CODE_USE_BEDROCK', type: 'boolean', description: '使用 AWS Bedrock' },
    ];
...
export function AdvancedSettings({
    ...
    const handleModeChange = useCallback((mode: RuntimeMode) => {
        onUpdateRuntimeConfig({
            engineModes: {
    ...
    const handleEnvVarChange = useCallback((key: keyof ClaudeEnvVars, value: string | number | boolean | undefined) => {
                ...
                <div className="p-4 space-y-4 border-t border-gray-200 dark:border-gray-700">
                                            ...
                                            {envVar.type === 'boolean' ? (
                                                <label className="flex items-center gap-2 cursor-pointer">
                                                    <input
                                                        ...
                                                        onChange={(e) => handleEnvVarChange(envVar.key, e.target.checked || undefined)}
                                                ...
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        onChange={(e) => handleEnvVarChange(envVar.key, e.target.value || undefined)}
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => handleEnvVarChange(envVar.key, e.target.value ? Number(e.target.value) : undefined)}
                                                    placeholder={envVar.placeholder}
                                                    className={cn(
                    ...
                    <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                        <p className="text-xs text-yellow-700 dark:text-yellow-300">
                            {engine === 'claude' && '提示: Claude Code CLI 需要在 WSL 中运行以获得最佳体验。环境变量会自动同步到 ~/.claude/settings.json'}
