...
import { CheckingView, DoneView, ErrorView, InstallingView } from "./StateViews";
...
export function DependencyChecker({
  ...
  const resolveVersionFallback = useCallback(
    async (binaryPath: string, args: string) => {
      try {
        const result = await executeCommand(buildVersionCommand(binaryPath, args), {
  ...
  const checkCli = useCallback(async (): Promise<DependencyStatus["cli"]> => {
    try {
      ...
      if (binaryResult?.installed) {
        ...
        if (!version) {
          const fallback = await executeCommand(`${cliConfig.command} ${cliConfig.versionArg}`, {
            timeoutMs: CHECK_TIMEOUT_MS,
          });
          if (fallback.success) {
            version = fallback.stdout.trim();
      ...
      const result = await executeCommand(`${cliConfig.command} ${cliConfig.versionArg}`, {
        timeoutMs: CHECK_TIMEOUT_MS,
      });
  ...
  const runCheck = useCallback(async () => {
    ...
    try {
      const [nodejs, npm, cli] = await Promise.all([checkNodejs(), checkNpm(), checkCli()]);
  ...
  const showManualRetry = useMemo(() => !isCheckError && !canRetry, [canRetry, isCheckError]);
  const showSkip = useMemo(() => !isCheckError && requiresCli, [isCheckError, requiresCli]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">环境检测</h4>
        <button
          onClick={runCheck}
        ...
        >
          <RefreshCw className={cn("w-3 h-3", state.phase === "CHECKING" && "animate-spin")} />
          重新检测
        </button>
      </div>
