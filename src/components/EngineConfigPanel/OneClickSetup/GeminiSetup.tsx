/**
 * Gemini CLI 配置组件
 * 处理 Gemini CLI 的安装和 Google 账号登录
 */

import { useState, useCallback } from "react";
import { CheckCircle, Loader2, ExternalLink, LogIn } from "lucide-react";
...
export function GeminiSetup({
  ...
  const handleLogin = useCallback(async () => {
    setIsLoggingIn(true);
    setVerifyError(null);
    onLog("启动 Google 账号登录...");

    try {
      // 执行 gemini auth login 命令
  ...
  const handleOpenDocs = useCallback(async () => {
    await open("https://www.geminicli.net/en/blog/gemini-cli-npm-installation-guide");
  }, []);
  ...
  switch (currentStep) {
    ...
    case "login":
      return (
        <div className="space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            使用 Google 账号登录以使用 Gemini CLI。
          </p>
          <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <p className="text-sm text-blue-700 dark:text-blue-300 mb-3">
              点击下方按钮将打开浏览器进行 Google 账号登录。登录完成后返回此页面继续。
            </p>
          ...
          {verifyError && (
            <p className="text-sm text-yellow-600 dark:text-yellow-400">{verifyError}</p>
          )}
          <button
    ...
    case "verify":
      return (
        <div className="space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-400">验证 Gemini CLI 是否正常工作。</p>
          <button
