...
export function SetupWizard({ engine, onComplete, onCancel }: SetupWizardProps) {
  ...
  // 处理依赖检测完成
  const handleDependencyCheckComplete = useCallback(
    (status: DependencyStatus) => {
      setDependencyStatus(status);

      // 检查是否满足要求
      const depsOk =
        status.nodejs.installed && status.nodejs.meetsRequirement && status.npm.installed;
  ...
  const renderStepContent = () => {
    ...
    if (currentStep.id === "check_deps") {
      return <DependencyChecker engine={engine} onCheckComplete={handleDependencyCheckComplete} />;
    }
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-lg overflow-hidden">
      ...
      <div className="flex">
        ...
        <div className="w-64 p-4 border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/30">
          <StepIndicator steps={stepsWithStatus} currentStep={currentStepIndex} />
        </div>
