...
export function InstallingView({ logs }: InstallingViewProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {lastLog && <p className="text-xs text-gray-500 dark:text-gray-400">当前步骤：{lastLog}</p>}
      {logs.length > 0 && (
...
function DependencyItem({
  ...
  return (
    <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
      ...
      <div className="flex items-center gap-2">
        {version && <span className="text-xs text-gray-500 dark:text-gray-400">{version}</span>}
        {!installed && <span className="text-xs text-red-500">未安装</span>}
        {installed && meetsRequirement === false && (
          <span className="text-xs text-yellow-500">{requirementText || "版本不满足"}</span>
        )}
      </div>
    </div>
  );
      ...
      {showSuccess && (
        <div className="p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm text-green-700 dark:text-green-300">环境检测通过，可以继续配置</p>
          </div>
        </div>
      )}
    </div>
