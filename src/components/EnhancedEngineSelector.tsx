...
export const EnhancedEngineSelector: React.FC<EnhancedEngineSelectorProps> = ({
  ...
  const getEngineAvailability = useCallback(
    (engine: ExecutionEngine): { available: boolean; reason?: string } => {
      switch (engine) {
        case "claude":
          return {
            available: claudeInstalled,
            reason: claudeInstalled ? undefined : "Claude Code CLI 未安装",
          };
        case "codex":
          return {
            available: codexAvailable,
            reason: codexAvailable ? undefined : "Codex CLI 未安装",
          };
        case "gemini":
          return {
            available: geminiAvailable,
            reason: geminiAvailable ? undefined : "Gemini CLI 未安装",
          };
        default:
  ...
  const handleCodexModeChange = useCallback(
  ...
  const handleGeminiApprovalModeChange = useCallback(
    (mode: "auto_edit" | "yolo" | "default") => {
  ...
  const handleRuntimeModeChange = useCallback(
    async (engine: "claude" | "codex" | "gemini", mode: RuntimeMode) => {
      const configMap = {
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className={cn(
                ...
                isSelected
                  ? [engine.bgClass, engine.borderClass, "ring-2 ring-offset-1"]
                  : ["bg-background/50 border-border/50", "hover:border-border hover:bg-accent/30"],
          ...
          <TooltipContent side="bottom" className="max-w-[200px]">
            <div className="space-y-1">
              ...
              {!available && reason && <p className="text-xs text-destructive">{reason}</p>}
            </div>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
  ...
  const renderRuntimeSelector = (engine: "claude" | "codex" | "gemini", config: any) => {
