...
const TEMPLATE_SUGGESTIONS: Record<string, string[]> = {
  afterCodeChange: ["运行测试验证更改", "提交这些更改", "检查是否有其他需要修改的地方"],
  afterError: ["修复这个错误", "解释这个错误的原因", "查看相关日志"],
  afterCompletion: ["还有什么需要完成的吗", "总结一下刚才的更改", "运行完整测试套件"],
  afterQuestion: ["请详细解释", "给个代码示例", "应用到当前项目"],
};
...
function extractRichContext(content: any[] | undefined): string {
  ...
  for (const block of content) {
    if (block.type === "text" && block.text) {
      ...
      parts.push(
        isError ? `[工具错误] ${resultContent}` : `[工具结果] ${resultContent.slice(0, 200)}`
      );
    }
  }
...
function extractFullSessionContext(messages: ClaudeStreamMessage[]): string {
  ...
  for (const msg of meaningfulMessages) {
    if (msg.type === "user" && msg.message) {
      const text =
        extractRichContext(msg.message.content) || extractTextFromContent(msg.message.content);
      if (text) {
...
export function usePromptSuggestion({
  ...
  const generateAISuggestion = useCallback(
    async (requestId: number): Promise<PromptSuggestion | null> => {
      try {
        ...
        const response = await claudeSDK.sendMessage([{ role: "user", content: userMessage }], {
          model,
          maxTokens: 60,
          temperature: 0.3,
  ...
  useEffect(() => {
    ...
    if (currentPrompt.length > 0) {
      ...
      if (
        currentSuggestion &&
        !currentSuggestion.text.toLowerCase().startsWith(currentPrompt.toLowerCase())
      ) {
        setSuggestion(null);
      }
    }
