...
export const HookToggleManager: React.FC<HookToggleManagerProps> = ({ onBack, className }) => {
      ...
      >
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              {hook.eventType && (
                <Badge variant="secondary" className="text-xs">
                  {HOOK_EVENT_TYPES.find((e) => e.value === hook.eventType)?.label ||
                    hook.eventType}
                </Badge>
              )}
            </div>
            ...
            {hook.description && (
              <p className="text-sm text-muted-foreground line-clamp-2 mt-1">{hook.description}</p>
            )}
    ...
    <div className={`flex flex-col h-full ${className}`}>
      ...
      <div className="mx-4 mt-4 p-3 bg-muted/50 rounded-md flex items-start gap-2">
        ...
        <div className="text-sm text-muted-foreground">
          <p>
            Hook 文件位于 <code className="bg-muted px-1 rounded">~/.claude/hooks/</code> 目录
          </p>
          <p className="mt-1">
            启用 Hook 后会自动添加到 <code className="bg-muted px-1 rounded">settings.json</code> 的
            hooks 配置中
          </p>
        </div>
      </div>
      ...
      <ScrollArea className="flex-1 p-4">
        {loading ? (
          ...
          <div className="space-y-3">{hooks.map(renderHookCard)}</div>
        )}
      </ScrollArea>
        ...
        <DialogContent>
          ...
          <div className="py-4">
            ...
            <Select
              ...
              onValueChange={(value) => setEventDialog({ ...eventDialog, selectedEvent: value })}
            >
              <SelectTrigger>
