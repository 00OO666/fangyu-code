...
export const ProjectList: React.FC<ProjectListProps> = ({
  ...
  const [geminiSessionsMap, setGeminiSessionsMap] = useState<Map<string, GeminiSessionInfo[]>>(
    new Map()
  );
  ...
  const getSessionBreakdown = (
    project: Project
  ): { claude: number; codex: number; gemini: number; total: number } => {
          ...
          {currentProjects.map((project) => {
            ...
            return (
              <div
                ...
                aria-label={t("projectList.projectLabel", {
                  projectName,
                  sessionCount,
                  createdAt: formatAbsoluteDateTime(project.created_at),
                })}
              >
                {/* 主要信息区：项目图标 + 项目名称 */}
                <div
                  className={cn(
                    ...
                    viewMode === "grid" ? "mb-3" : "flex-1 items-center mb-0"
                  )}
                >
                  <div
                    className={cn(
                  ...
                  <div
                    className={cn(
                      ...
                      viewMode === "grid" ? "flex-1 pr-16" : "flex-1"
                    )}
                  >
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent side="left" className="p-0 border-primary/20 shadow-xl">
                          <div className="px-3 py-2 space-y-2 min-w-[150px]">
                            <p className="text-xs font-semibold text-foreground border-b border-border/50 pb-1.5">
                              {t("projectList.sessionStats")}
                            </p>
                    ...
                    >
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {onProjectSettings && onProjectDelete && <DropdownMenuSeparator />}
                          {onProjectDelete && (
                            <DropdownMenuItem
                              onClick={(e) => {
                                e.stopPropagation();
