...
export const PromptEnhancementSettings: React.FC<PromptEnhancementSettingsProps> = ({
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{
    show: boolean;
    provider: PromptEnhancementProvider | null;
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{t("promptEnhancement.subtitle")}</p>
          </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground space-y-1">
                      <div>
                        {t("promptEnhancement.modelName")}: {provider.model}
                      </div>
                      <div className="truncate">API: {provider.apiUrl}</div>
                      <div className="flex items-center gap-2">
                        <span>
                          {t("promptEnhancement.apiFormat")}:{" "}
                          {provider.apiFormat
                            ...
                            : `${t("promptEnhancement.autoDetect")} (${
                                detectApiFormat(provider.apiUrl) === "gemini"
                                  ? "Gemini"
                                  : detectApiFormat(provider.apiUrl) === "anthropic"
                                    ? "Anthropic"
                                    : "OpenAI"
                              })`}
                        </span>
                        ...
                        {provider.maxTokens !== undefined && (
                          <span>| Token: {provider.maxTokens}</span>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => handleDelete(provider)}>
          ...
          <DialogHeader>
            <DialogTitle>
              {editingProvider && providers.find((p) => p.id === editingProvider.id)
                ? t("promptEnhancement.editProvider")
                : t("promptEnhancement.addProvider")}
            </DialogTitle>
          </DialogHeader>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setEditingProvider({
                      ...editingProvider,
                      apiFormat:
                        value === "auto" ? undefined : (value as "openai" | "gemini" | "anthropic"),
                    })
                  }
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="auto">
                      ...
                      {editingProvider.apiUrl
                        ? `(${
                            detectApiFormat(editingProvider.apiUrl) === "gemini"
                              ? "Gemini"
                              : detectApiFormat(editingProvider.apiUrl) === "anthropic"
                                ? "Anthropic"
                                : "OpenAI"
                          })`
                        : ""}
                    ...
                    <SelectItem value="anthropic">
                      {t("promptEnhancement.anthropicFormat")}
                    </SelectItem>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="py-4">
            <p className="text-sm text-muted-foreground">
              {t("promptEnhancement.confirmDeleteMessage").replace(
                "{name}",
                deleteConfirm.provider?.name || ""
              )}
            </p>
