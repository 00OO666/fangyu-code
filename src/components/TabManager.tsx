    ...
    <TooltipProvider>
      <div className={cn("h-full flex flex-col bg-background", className)}>
        ...
        <div className="flex-shrink-0 border-b border-border bg-background">
          <div className="flex items-center h-12 px-2 sm:px-4 gap-1 sm:gap-2">
              ...
              {tabs.map((tab, index) => {
                ...
                const hasGlobalTask = tab.session?.id
                  ? Array.from(globalTaskState.tasks.values()).some(
                      (task) =>
                        (task.tabId === tab.id || task.sessionId === tab.session?.id) &&
                      ...
                      <div className="space-y-1.5 text-xs">
                        ...
                        {tab.session && (
                          <>
                            ...
                            <div className="text-muted-foreground">
                              {t("tabs.createdAt")}{" "}
                              {new Date(tab.session.created_at * 1000).toLocaleString("zh-CN")}
                            </div>
                          </>
                        )}
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    <span className="hidden md:inline text-sm text-muted-foreground truncate max-w-[120px] lg:max-w-none">
                      搜索 MCP/Skill...
                    </span>
            ...
            <UnifiedSearchPanel
              ...
              projectPath={
                tabs.find((t) => t.isActive)?.projectPath ||
                tabs.find((t) => t.isActive)?.session?.project_path
              }
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem
                  onClick={() =>
                    tabs.filter((tab) => !tab.isActive).forEach((tab) => closeTab(tab.id, true))
                  }
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {tabs.map((tab) => {
            return (
              <div key={tab.id} className={cn("absolute inset-0", !tab.isActive && "hidden")}>
                <TabSessionWrapper
                  tabId={tab.id}
                  session={tab.session}
            ...
            >
              <div className="text-center max-w-md px-8">
                ...
                >
                  ...
                  <Button size="lg" variant="outline" onClick={onBack} className="w-full">
        ...
        <Dialog open={Boolean(tabToClose)} onOpenChange={(open) => !open && setTabToClose("")}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>{t("tabs.unsavedChangesWarning")}</DialogDescription>
            </DialogHeader>
            <DialogFooter>
