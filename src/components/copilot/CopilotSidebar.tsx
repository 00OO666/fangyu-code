...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message, onApplyCode }) => {
  ...
  return (
    <motion.div
      ...
      className={cn("flex gap-3 p-3 rounded-lg", isUser ? "bg-primary/5" : "bg-muted/30")}
    >
      {/* 头像 */}
      <div
        className={cn(
  ...
  const handleSend = useCallback(async () => {
    ...
    const aiMessage: CopilotMessage = {
      ...
      codeBlocks: inputValue.includes("/")
        ...
        : undefined,
      suggestions: [QUICK_ACTIONS[0], QUICK_ACTIONS[1]],
    };
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
          ...
          <div className="p-3 border-t bg-muted/20">
            ...
            <div className="flex items-end gap-2">
              ...
              <div className="flex flex-col gap-1">
                ...
                {messages.length > 0 && (
                  <Button variant="ghost" size="sm" className="h-8 px-3" onClick={handleClear}>
        ...
        <TabsContent value="history" className="flex-1 m-0 p-3">
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs text-muted-foreground">历史对话记录将显示在这里</p>
          </div>
        </TabsContent>
      </Tabs>
    </motion.div>
