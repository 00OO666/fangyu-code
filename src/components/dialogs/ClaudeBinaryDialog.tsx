      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription className="space-y-3 mt-4">
            <p>{t("claudeBinaryDialog.description")}</p>
            <div className="flex items-center gap-2 p-3 bg-muted rounded-md">
              <AlertCircle className="w-4 h-4 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">
                <span className="font-medium">{t("claudeBinaryDialog.searchLocations")}：</span>{" "}
        ...
        <DialogFooter className="gap-3">
          ...
          <Button onClick={() => onOpenChange(false)}>{t("claudeBinaryDialog.gotIt")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
