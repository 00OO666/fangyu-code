    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="sm:max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <DialogDescription>Claude 已完成规划，请审批以下计划</DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="flex-1 min-h-0 my-4">
          <div className="text-sm font-medium text-muted-foreground mb-2">计划内容：</div>
          <ScrollArea className="h-[300px] rounded-lg border bg-muted/30 p-4">
            <div className="prose prose-sm dark:prose-invert max-w-none">
              <ReactMarkdown>{plan || "（无计划内容）"}</ReactMarkdown>
            </div>
          </ScrollArea>
        </div>
        ...
        <div className="text-xs text-muted-foreground bg-muted/50 rounded-lg p-3 mb-4">
          ...
          <ul className="list-disc list-inside space-y-1">
            <li>
              <strong>批准执行</strong>：关闭 Plan 模式，Claude 将开始执行计划中的操作
            </li>
            <li>
              <strong>继续规划</strong>：保持 Plan 模式，你可以要求 Claude 修改或完善计划
            </li>
          </ul>
        </div>
