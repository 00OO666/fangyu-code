...
export const SummaryEngineConfig: React.FC<SummaryEngineConfigProps> = ({
  ...
  const testStats = useMemo(() => {
    ...
    const available = testedModels.filter(
      (m) => m.testStatus === "success" || m.testStatus === "replaced"
    ).length;
  ...
  const handleTestAllModels = useCallback(async () => {
    ...
    try {
      const result = await testEngineModels(selectedEngine, apiKey, apiEndpoint, (progress) =>
  ...
  const renderEngineCard = (engineId: SummaryEngine) => {
    ...
    return (
      ...
      >
        ...
        <span
          className={cn("text-xs font-medium", isSelected ? engine.textClass : "text-foreground")}
        >
          {engine.name}
        </span>
      </button>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>配置用于生成会话摘要的 AI 引擎，独立于主聊天引擎</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">{currentEngine.description}</p>
          </div>
            ...
            {hasTestedCurrentEngine && !isTesting && (
              <div className="flex items-center gap-4 text-xs">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Switch checked={showAllModels} onCheckedChange={setShowAllModels} />
                </div>
              </div>
            )}
          </div>
          ...
          {selectedEngine === "claude" && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-orange-500/10 border border-orange-500/20">
              ...
              <p className="text-xs text-orange-700 dark:text-orange-300">
                Claude 引擎使用当前 Claude Code 的配置，无需额外设置 API Key。 摘要将通过 Tauri
                后端调用。
              </p>
            </div>
          )}
