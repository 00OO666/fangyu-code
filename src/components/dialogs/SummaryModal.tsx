    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>生成当前会话的摘要，可复制到剪贴板或在新会话中继续</DialogDescription>
        </DialogHeader>
          ...
          <div className="flex items-center justify-between p-3 border rounded-lg bg-muted/30">
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  backgroundColor: `${ENGINE_DISPLAY_INFO[savedConfig?.engine || "claude"].color}20`,
                }}
              >
                <span
        ...
        <DialogFooter className="flex-shrink-0 gap-2">
          {!result ? (
            <>
              ...
              <Button onClick={handleGenerate} disabled={isGenerating || messages.length === 0}>
                {isGenerating ? (
                  <>
            ...
            <>
              ...
              <Button variant="outline" onClick={handleOpenInNewSession} disabled={!result.success}>
...
const StatCard: React.FC<StatCardProps> = ({ icon, label, value, subValue, warning }) => (
  <div
    className={cn("p-2 rounded-lg border bg-card", warning && "border-amber-500/50 bg-amber-500/5")}
  >
    ...
    <div className="flex items-baseline gap-1">
      <span className={cn("text-sm font-medium", warning && "text-amber-500")}>{value}</span>
