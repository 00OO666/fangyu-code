...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const NavButton = ({ item }: { item: NavItem }) => {
    ...
    const buttonContent = (
      ...
      >
        <div
          className={cn(
            "absolute inset-0 bg-gradient-to-r from-[var(--accent-orange)]/10 via-[var(--accent-gold)]/5 to-transparent opacity-0 transition-opacity duration-300",
            (isHovered || isActive) && "opacity-100"
          )}
        />
        {isActive && (
          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-[var(--accent-gradient)] rounded-r-full shadow-[var(--glow-orange)]" />
        )}
        <item.icon
          className={cn(
            "w-4 h-4 flex-shrink-0 transition-transform duration-300 relative z-10",
            isActive && "scale-110",
        ...
        {isExpanded && (
          <span
            className={cn(
              "ml-3 text-sm font-medium whitespace-nowrap overflow-hidden text-ellipsis relative z-10 transition-all duration-300",
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "flex items-center gap-3 mb-4 pb-3 border-b border-white/10",
          !isExpanded && "justify-center"
        )}
      >
        <Avatar
          className={cn(
            "transition-all duration-300 ring-2 ring-[var(--accent-orange)]/30 ring-offset-2 ring-offset-transparent",
            isExpanded ? "h-10 w-10" : "h-8 w-8"
          )}
        >
          <AvatarImage src="/avatars/fangyu.png" alt="Fangyu" />
          <AvatarFallback className="bg-[var(--accent-gradient)] text-white font-bold">
            FY
          </AvatarFallback>
        </Avatar>
        {isExpanded && (
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold truncate text-white">Fangyu</p>
            <p className="text-xs text-white/60 truncate">开发者</p>
          </div>
        )}
      </div>
      {/* 快捷搜索按钮 */}
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              variant="outline"
              className={cn(
                "mb-4 transition-all duration-300 light-glass hover:medium-glass",
                isExpanded ? "w-full justify-start px-3 h-9" : "w-10 h-10"
              )}
              onClick={() => {
      ...
      <div
        className={cn(
          "flex-1 flex flex-col w-full min-h-0 overflow-y-auto scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent",
          isExpanded ? "space-y-0.5" : "items-center space-y-1"
        )}
      >
      ...
      <div
        className={cn(
          "flex flex-col w-full mt-auto pt-3 border-t border-white/10 flex-shrink-0",
          isExpanded ? "space-y-2" : "items-center space-y-2"
        )}
      >
        <div className={cn(isExpanded ? "w-full" : "flex justify-center w-full")}>
          <UnifiedEngineStatus compact={!isExpanded} />
        </div>
        {isExpanded && (
          <div className="px-1">
        ...
        <div
          className={cn("flex items-center gap-1", isExpanded ? "justify-around px-1" : "flex-col")}
        >
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
        ...
        <div
          className={cn(
            "flex items-center gap-1 pt-2 border-t border-white/10",
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className="w-8 h-8 text-white/70 hover:text-white transition-transform duration-300"
                  aria-label={
      ...
      {isExpanded && (
        <div
          className={cn(
            "absolute right-0 top-0 bottom-0 w-1 cursor-ew-resize transition-colors hover:bg-[var(--accent-orange)]/30",
            isDragging && "bg-[var(--accent-orange)]/50"
          )}
