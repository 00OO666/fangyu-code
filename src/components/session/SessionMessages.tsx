...
export const SessionMessages = forwardRef<SessionMessagesRef, SessionMessagesProps>(
  ({ messageGroups, isLoading, error, parentRef, onCancel }, ref) => {
          ...
          if (group.type === "normal") {
            ...
            if (messageType === "user") {
              ...
              if (typeof content === "string") {
                hasText = (content as string).trim().length > 0;
              } else if (Array.isArray(content)) {
                hasText = content.some((item: any) => item.type === "text" && item.text?.trim());
              }
    ...
    return (
      <div ref={parentRef} className="flex-1 overflow-y-auto relative" data-virtual-list="true">
        <div
        ...
        >
          {rowVirtualizer.getVirtualItems().map((virtualItem) => {
            ...
            if (!messageGroup) {
              logger.warn(
                "SessionMessages",
                "[SessionMessages] messageGroup is undefined for index:",
                virtualItem.index
              );
              return null;
            }
            ...
            const messagesIndex =
              originalIndex !== undefined && displayableToMessagesIndexMap
                ? displayableToMessagesIndexMap.get(originalIndex)
                : undefined;
            const promptIndex =
              message &&
              message.type === "user" &&
              messagesIndex !== undefined &&
              getPromptIndexForMessage
        ...
        <div className="sticky bottom-0 left-0 right-0 z-10 pointer-events-none">
          <div className="pointer-events-auto">
            <CliProcessingIndicator isProcessing={isLoading} onCancel={onCancel} />
          </div>
        </div>
      </div>
    );
