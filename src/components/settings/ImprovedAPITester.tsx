...
export const ImprovedAPITester: React.FC<ImprovedAPITesterProps> = ({
    ...
    const getStatusColor = (status: ModelTestResult['status']) => {
        switch (status) {
            ...
            case 'replaced': return 'text-yellow-500';
                                        ...
                                        >
                                            ...
                                            <div className="text-right ml-3">
                                                ...
                                                {result.status === 'success' && (
                                                    <span className={`text-sm font-medium ${getLatencyColor(result.latency)}`}>
                                                        {result.latency}ms
                                                    </span>
                                                )}
                                                ...
                                                {(result.status === 'error' || result.status === 'timeout') && (
                                                    <p className="text-xs text-red-500 max-w-[250px] truncate" title={result.error}>
                                                        {result.error}
                                                    </p>
                                                )}
                                            </div>
