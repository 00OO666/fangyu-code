...
export const SuperAgentSettings: React.FC = () => {
  ...
  const [defaultAutonomyMode, setDefaultAutonomyMode] = useState<"supervised" | "autopilot">(
    "supervised"
  );
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>配置多 Agent 编排系统的行为</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>最大并发 Agent 数</Label>
              <p className="text-xs text-muted-foreground">同时运行的后台 Agent 数量上限</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>默认自治模式</Label>
              <p className="text-xs text-muted-foreground">新会话的默认自治模式</p>
            </div>
            <Select
              value={defaultAutonomyMode}
              onValueChange={(v: "supervised" | "autopilot") => setDefaultAutonomyMode(v)}
            >
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>启用后台任务</Label>
              <p className="text-xs text-muted-foreground">允许 Agent 在后台执行长时间任务</p>
            </div>
            <Switch checked={enableBackgroundTasks} onCheckedChange={setEnableBackgroundTasks} />
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>自动批准级别</Label>
              <p className="text-xs text-muted-foreground">自动驾驶模式下自动批准的操作风险级别</p>
            </div>
            <Select
              value={autoApproveLevel}
              onValueChange={(v: "none" | "low" | "medium") => setAutoApproveLevel(v)}
            >
              <SelectTrigger className="w-40">
                <SelectValue />
              </SelectTrigger>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>配置智能上下文管理和 Token 优化</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>警告阈值</Label>
              <p className="text-xs text-muted-foreground">上下文使用量达到此比例时显示警告</p>
            </div>
            <div className="flex items-center gap-2">
              <Slider
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>临界阈值</Label>
              <p className="text-xs text-muted-foreground">上下文使用量达到此比例时触发压缩</p>
            </div>
            <div className="flex items-center gap-2">
              <Slider
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>自动压缩</Label>
              <p className="text-xs text-muted-foreground">达到临界阈值时自动压缩上下文</p>
            </div>
            <Switch checked={enableAutoCompaction} onCheckedChange={setEnableAutoCompaction} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>配置安全防护和审计功能</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>启用安全防护</Label>
              <p className="text-xs text-muted-foreground">启用路径验证和命令安全检查</p>
            </div>
            <Switch checked={enableSecurityGuard} onCheckedChange={setEnableSecurityGuard} />
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">自动拦截 rm -rf、format 等危险命令</p>
            </div>
            <Switch checked={blockDangerousCommands} onCheckedChange={setBlockDangerousCommands} />
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>审计日志</Label>
              <p className="text-xs text-muted-foreground">记录所有 Agent 操作用于审计</p>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>配置 AI 模型提供商和 API 密钥</CardDescription>
        </CardHeader>
        <CardContent>
          <APIConfigPanel />
        </CardContent>
