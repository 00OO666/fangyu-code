...
function isDialogAccessible(config: DialogConfig): boolean {
    ...
    const hasValidDescription = config.hasDescription &&
        config.descriptionText != null &&
        config.descriptionText.trim() !== '';

    const hasValidAriaDescribedBy = config.hasAriaDescribedBy &&
...
describe('Property 3: Dialog Accessibility', () => {
    it('accessible dialogs should pass validation', () => {
        fc.assert(
            fc.property(
                generateAccessibleDialogConfig(),
                (config) => {
    ...
    it('inaccessible dialogs should fail validation', () => {
        fc.assert(
            fc.property(
                generateInaccessibleDialogConfig(),
                (config) => {
                    const result = isDialogAccessible(config);
                    return result === false;
                }
