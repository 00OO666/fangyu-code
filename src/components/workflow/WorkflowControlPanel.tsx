...
import type { WorkflowDAG, Agent, WorkflowLog, WorkflowConfig } from "@/core/types/workflow";
  ...
  return (
    <Card className="border-2 border-dashed">
      <CardHeader className="pb-3">
        ...
        <CardDescription>描述您要完成的任务，AI 将自动分解为可并行执行的工作流</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button onClick={onSubmit} disabled={!value.trim() || isLoading} className="gap-2">
            {isLoading ? (
              <>
...
const StatsPanel: React.FC<{ workflow: WorkflowDAG | null; agents: Agent[] }> = ({
  ...
  if (!stats) {
    return <div className="text-center text-muted-foreground py-8">等待工作流生成...</div>;
  }
...
export const WorkflowControlPanel: React.FC<WorkflowControlPanelProps> = ({
  ...
  const handleGenerateWorkflow = useCallback(async () => {
    ...
    try {
      ...
      addLog(
        "info",
        `工作流生成完成: ${mockWorkflow.tasks.length} 个任务, ${mockWorkflow.parallelGroups.length} 个并行组`
      );
    } catch (error: any) {
      setState((prev) => ({
        ...prev,
