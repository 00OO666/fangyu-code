...
import { TaskQueue, createPriorityQueue, createFifoQueue, createDeadlineQueue } from "./TaskQueue";
...
describe("TaskQueue Property Tests", () => {
  ...
  describe("Property 4.3: 依赖未满足的任务不会出队", () => {
    ...
    it("dependency chain is respected", () => {
      fc.assert(
        fc.property(fc.integer({ min: 2, max: 5 }), (chainLength) => {
          const queue = new TaskQueue();
          const tasks: Task[] = [];
  ...
  describe("Property 4.5: 任务唯一性", () => {
    it("duplicate task IDs are rejected", () => {
      fc.assert(
        fc.property(fc.uuid(), fc.integer({ min: 2, max: 10 }), (taskId, duplicateCount) => {
          const queue = new TaskQueue();
          let successCount = 0;
