...
const retryConfigArb: fc.Arbitrary<Partial<RetryConfig>> = fc.record({
  ...
  backoffMultiplier: fc.option(fc.float({ min: Math.fround(1.1), max: Math.fround(3), noNaN: true }), { nil: undefined }),
  jitterFactor: fc.option(fc.float({ min: Math.fround(0), max: Math.fround(0.5), noNaN: true }), { nil: undefined }),
});
