...
const arbOperationType: fc.Arbitrary<OperationType> = fc.constantFrom(
  'file_create', 'file_modify', 'file_delete', 'command_execute',
  'git_commit', 'git_push', 'install_package', 'config_change', 'network_request'
);
...
describe('AutonomyController Property Tests', () => {
  ...
  describe('Configuration Management', () => {
    ...
    it('should manage confirmation requirements', () => {
      fc.assert(
        fc.property(
          fc.array(arbOperationType, { minLength: 1, maxLength: 5 }),
          (types) => {
            const ctrl = new AutonomyController({
