...
describe("SmartContextManager Property Tests", () => {
  ...
  describe("Context Injection Properties (Property 11)", () => {
    ...
    it("Property 11.3: 不同内容应都被注入", () => {
      fc.assert(
        fc.property(fc.array(contentArb, { minLength: 2, maxLength: 10 }), (contents) => {
          const freshManager = new SmartContextManager();
    ...
    it("Property 11.4: 批量注入应正确去重", () => {
      fc.assert(
        fc.property(fc.array(contextItemInputArb, { minLength: 2, maxLength: 10 }), (inputs) => {
          const freshManager = new SmartContextManager();
  ...
  describe("Threshold Detection Properties (Property 10)", () => {
    ...
    it("Property 10.4: 使用率计算应准确", () => {
      fc.assert(
        fc.property(fc.array(contentArb, { minLength: 1, maxLength: 20 }), (contents) => {
          const freshManager = new SmartContextManager();
  ...
  describe("Compaction Properties (Property 12)", () => {
    it("Property 12.1: 压缩应减少 token 数量", () => {
      fc.assert(
        fc.property(fc.array(contextItemInputArb, { minLength: 5, maxLength: 20 }), (inputs) => {
          const freshManager = new SmartContextManager("claude-3-sonnet", {
            autoCompact: false,
          });
          ...
          const uniqueInputs = inputs.filter(
            (input, index, self) => self.findIndex((i) => i.content === input.content) === index
          );
    ...
    it("Property 12.2: system 类型应在压缩中保留", () => {
      fc.assert(
        fc.property(fc.array(contentArb, { minLength: 3, maxLength: 10 }), (contents) => {
          const freshManager = new SmartContextManager("claude-3-sonnet", {
            autoCompact: false,
          });
          ...
          freshManager.inject({
            type: "system",
            content: "SYSTEM: " + uniqueContents[0],
            priority: 1,
          });
          ...
          (priorities) => {
            ...
            if (result.removed.length > 0 && result.retained.length > 0) {
              ...
              if (retainedUserItems.length > 0 && removedUserItems.length > 0) {
                const retainedUserMinPriority = Math.min(
                  ...retainedUserItems.map((i) => i.priority)
                );
  ...
  describe("Model Configuration Properties", () => {
    it("Property 10.5: 切换模型应更新配置", () => {
      fc.assert(
        fc.property(fc.constantFrom(...Object.keys(MODEL_CONFIGS)), (modelName) => {
          const freshManager = new SmartContextManager();
  ...
  describe("Statistics Properties", () => {
    it("Property 10.6: 统计信息应准确反映状态", () => {
      fc.assert(
        fc.property(fc.array(contextItemInputArb, { minLength: 1, maxLength: 10 }), (inputs) => {
          const freshManager = new SmartContextManager();
  ...
  describe("Context Building Properties", () => {
    it("Property 10.8: 构建的上下文应包含所有内容", () => {
      fc.assert(
        fc.property(fc.array(contentArb, { minLength: 1, maxLength: 5 }), (contents) => {
          const freshManager = new SmartContextManager();
