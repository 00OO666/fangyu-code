...
export class SmartContextManager {
  ...
  constructor(modelName: string = "claude-3-sonnet", thresholdConfig?: Partial<ThresholdConfig>) {
    this.modelConfig = MODEL_CONFIGS[modelName] ?? MODEL_CONFIGS["claude-3-sonnet"];
    this.thresholdConfig = {
      warning: thresholdConfig?.warning ?? 0.7,
  ...
  triggerCompaction(strategy: CompactionStrategy = "hybrid"): CompactionResult {
    ...
    const _mustRetain = sortedItems.filter(
      (item) => item.type === "system" || (item.type === "steering" && item.priority <= 10)
    );
    const canRemove = sortedItems.filter(
      (item) => item.type !== "system" && !(item.type === "steering" && item.priority <= 10)
    );
   ...
   */
  buildContext(): string {
    const items = Array.from(this.items.values()).sort((a, b) => a.priority - b.priority);
