...
import { StrReplaceOptions, StrReplaceResult, FileEncoding } from "../types/unified-agent";
...
export class PreciseFileOps {
   ...
   * 在指定行插入内容
   */
  async insertAtLine(path: string, lineNumber: number, content: string): Promise<boolean> {
   ...
   * 删除行范围
   */
  async deleteLines(path: string, startLine: number, endLine: number): Promise<boolean> {
    const fileContent = await this.fs.readFile(path);
    const lines = fileContent.split("\n");
