...
const hookActionArb: fc.Arbitrary<HookAction> = fc.record({
  type: fc.constantFrom<'sendMessage' | 'executeCommand' | 'custom'>('sendMessage', 'executeCommand', 'custom'),
...
describe('EnhancedHookEngine Property Tests', () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (enabledStates) => {
            ...
            // 只有启用的 hook 应该执行
            const executedCount = result.results.filter(r => 
              !r.output || !(r.output as { skipped?: boolean }).skipped
            ).length;
    ...
    it('Property 5.6: 只有匹配事件的 Hook 应被执行', async () => {
      await fc.assert(
        fc.asyncProperty(
          eventTypeArb,
          eventTypeArb,
          async (targetEvent, triggerEvent) => {
            const freshEngine = new EnhancedHookEngine(trackingExecutor);
            trackingExecutor.reset();
  ...
  describe('Engine State Properties', () => {
    ...
    it('Property 5.8: 清除后应没有任何 Hook', () => {
      fc.assert(
        fc.property(
          fc.array(hookDefinitionArb(), { minLength: 1, maxLength: 10 }),
          (hooks) => {
            const freshEngine = new EnhancedHookEngine();
