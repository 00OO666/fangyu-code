...
describe("SteeringLoader Property Tests", () => {
  ...
  describe("Context-based Loading", () => {
    ...
    it("Property 7.10: getForContext 应正确组合所有模式", async () => {
      // 创建三种模式的文件
      const alwaysContent = generateSteeringContent(
        { inclusion: "always", priority: 1 },
        "# Always"
      );
      ...
      const manualContent = generateSteeringContent(
        { inclusion: "manual", priority: 3 },
        "# Manual"
      );
      ...
      // 测试不同上下文组合
      fc.assert(
        fc.property(fc.boolean(), fc.boolean(), (includeActiveFile, includeManualRef) => {
