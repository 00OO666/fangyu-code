...
const earsPatternGenerators: Record<EARSPattern, fc.Arbitrary<string>> = {
  ...
  ).map(([option, system, action]) => `WHERE ${option}, THE ${system} SHALL ${action} additional features`),
  ...
  ).map(([option, state, trigger, system, action]) => 
...
// 生成非 EARS 格式的字符串
const arbNonEARSString = fc.string({ minLength: 1, maxLength: 100 })
  .filter(s => {
    const upper = s.toUpperCase();
    return !upper.startsWith('THE ') &&
  ...
  describe('Property 13: EARS Format Compliance', () => {
    ...
    it('should detect correct EARS pattern type', () => {
      fc.assert(
        fc.property(
          fc.oneof(
            earsPatternGenerators.ubiquitous.map(c => ({ criterion: c, expected: 'ubiquitous' as EARSPattern })),
            earsPatternGenerators['event-driven'].map(c => ({ criterion: c, expected: 'event-driven' as EARSPattern })),
            earsPatternGenerators['state-driven'].map(c => ({ criterion: c, expected: 'state-driven' as EARSPattern })),
            earsPatternGenerators.unwanted.map(c => ({ criterion: c, expected: 'unwanted' as EARSPattern })),
            earsPatternGenerators.optional.map(c => ({ criterion: c, expected: 'optional' as EARSPattern }))
          ),
          ({ criterion, expected }) => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (featureName, idea) => {
            ...
            const normalizedName = featureName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
