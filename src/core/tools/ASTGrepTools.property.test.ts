...
describe("ASTGrepTools Property Tests", () => {
      ...
      fc.assert(
        fc.property(
          fc
            ...
            .filter((s) => !SUPPORTED_LANGUAGES.includes(s as SupportedLanguage)),
          (lang) => {
  ...
  describe("Formatted Search Properties", () => {
    it("Property 8.12: 格式化搜索应包含匹配数量", async () => {
      await fc.assert(
        fc.asyncProperty(fc.array(astMatchArb, { minLength: 1, maxLength: 5 }), async (matches) => {
          // 为每次测试创建新的实例
          const executor = new MockASTGrepExecutor();
          const testTools = new ASTGrepTools("/workspace", executor);
  ...
  describe("Error Handling Properties", () => {
    it("Property 8.14: 不支持的语言应抛出错误", async () => {
      await expect(tools.search("pattern", "unsupported_lang")).rejects.toThrow(
    ...
    it("Property 8.15: 替换时不支持的语言应抛出错误", async () => {
      await expect(
        tools.replace("pattern", "replacement", "/path", "unsupported_lang")
  ...
  describe("Capture Replacement Properties", () => {
    it("Property 8.16: 捕获组应正确替换", async () => {
      ...
      const result = await tools.replace(
        "function $name()",
        "const $name = () =>",
        "/workspace",
        "typescript"
