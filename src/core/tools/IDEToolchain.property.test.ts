...
// 生成诊断
const diagnosticArb = (file: string): fc.Arbitrary<Diagnostic> => fc.record({
  ...
  code: fc.option(fc.oneof(fc.string({ minLength: 1, maxLength: 10 }), fc.integer({ min: 1, max: 9999 })), { nil: undefined })
});
...
describe('IDEToolchain Property Tests', () => {
  ...
  describe('Syntax Validation Properties (Property 9)', () => {
    it('Property 9.1: 无错误时验证应返回 valid=true', async () => {
      await fc.assert(
        fc.asyncProperty(
          filePathArb,
          fc.array(diagnosticArb('test.ts').filter(d => d.severity !== 'error'), { maxLength: 5 }),
    ...
    it('Property 9.2: 有错误时验证应返回 valid=false', async () => {
      await fc.assert(
        fc.asyncProperty(
          filePathArb,
          fc.array(diagnosticArb('test.ts'), { minLength: 1, maxLength: 5 })
    ...
    it('Property 9.3: 错误、警告、提示应正确分类', async () => {
      ...
      const diagnostics: Diagnostic[] = [
        { file: fullPath, range: { start: { line: 1, character: 0 }, end: { line: 1, character: 10 } }, message: 'Error 1', severity: 'error' },
        { file: fullPath, range: { start: { line: 2, character: 0 }, end: { line: 2, character: 10 } }, message: 'Error 2', severity: 'error' },
        { file: fullPath, range: { start: { line: 3, character: 0 }, end: { line: 3, character: 10 } }, message: 'Warning 1', severity: 'warning' },
        { file: fullPath, range: { start: { line: 4, character: 0 }, end: { line: 4, character: 10 } }, message: 'Hint 1', severity: 'hint' },
        { file: fullPath, range: { start: { line: 5, character: 0 }, end: { line: 5, character: 10 } }, message: 'Info 1', severity: 'info' }
      ];
    ...
    it('Property 9.4: 摘要应正确反映问题数量', async () => {
      ...
      lspClient.setDiagnostics('/workspace/warnings.ts', [
        { file: '/workspace/warnings.ts', range: { start: { line: 1, character: 0 }, end: { line: 1, character: 10 } }, message: 'Warning', severity: 'warning' }
      ]);
      ...
      lspClient.setDiagnostics('/workspace/errors.ts', [
        { file: '/workspace/errors.ts', range: { start: { line: 1, character: 0 }, end: { line: 1, character: 10 } }, message: 'Error', severity: 'error' },
        { file: '/workspace/errors.ts', range: { start: { line: 2, character: 0 }, end: { line: 2, character: 10 } }, message: 'Warning', severity: 'warning' }
      ]);
  ...
  describe('Code Analysis Properties (Property 9.5)', () => {
    it('Property 9.5.1: 分析应返回请求的信息', async () => {
      ...
      lspClient.setDefinition(fullPath, 5, 10, { file: 'other.ts', range: { start: { line: 1, character: 0 }, end: { line: 1, character: 10 } } });
      lspClient.setReferences(fullPath, 5, 10, [
        { file: 'ref1.ts', range: { start: { line: 1, character: 0 }, end: { line: 1, character: 10 } } }
      ]);
  ...
  describe('Code Search Properties (Property 9.6)', () => {
    it('Property 9.6.1: 搜索应返回正确的结果结构', async () => {
      const matches = [
        { file: '/workspace/test.ts', range: { start: { line: 1, character: 0 }, end: { line: 1, character: 10 } }, text: 'match1' }
      ];
  ...
  describe('Batch Validation Properties (Property 9.8)', () => {
    ...
    it('Property 9.8.2: 工作区摘要应正确统计', async () => {
      lspClient.setDiagnostics('/workspace/clean.ts', []);
      lspClient.setDiagnostics('/workspace/errors.ts', [
        { file: '/workspace/errors.ts', range: { start: { line: 1, character: 0 }, end: { line: 1, character: 10 } }, message: 'Error 1', severity: 'error' },
        { file: '/workspace/errors.ts', range: { start: { line: 2, character: 0 }, end: { line: 2, character: 10 } }, message: 'Error 2', severity: 'error' }
      ]);
      lspClient.setDiagnostics('/workspace/warnings.ts', [
        { file: '/workspace/warnings.ts', range: { start: { line: 1, character: 0 }, end: { line: 1, character: 10 } }, message: 'Warning', severity: 'warning' }
      ]);
      
      const summary = await toolchain.getWorkspaceDiagnosticsSummary(['clean.ts', 'errors.ts', 'warnings.ts']);
