...
fc.record({
  ...
  type: fc.constantFrom("sequential", "parallel", "conditional") as fc.Arbitrary<
    "sequential" | "parallel" | "conditional"
  >,
  priority: fc.constantFrom("low", "medium", "high", "critical") as fc.Arbitrary<
...
describe("UnifiedWorkflowEngine Property Tests", () => {
  describe("Property 1: DAG Validity", () => {
    ...
    it("generated DAG should have no cycles", () => {
      fc.assert(
        fc.property(fc.integer({ min: 1, max: 20 }), (taskCount) => {
  ...
  describe("Property 2: Parallel Task Identification", () => {
    ...
    it("tasks with no mutual dependencies can be parallelized", () => {
      fc.assert(
        fc.property(fc.integer({ min: 2, max: 10 }), (taskCount) => {
          // 创建完全独立的任务（无依赖）
          const tasks: Task[] = [];
          for (let i = 0; i < taskCount; i++) {
            tasks.push({
    ...
    it("entry points should all be parallelizable", () => {
      fc.assert(
        fc.property(fc.integer({ min: 1, max: 15 }), (taskCount) => {
          const dag = generateValidDAG(taskCount);
          const entryTasks = dag.tasks.filter((t) => dag.entryPoints.includes(t.id));

          // 入口任务之间不应该有依赖关系
          return entryTasks.every((t) =>
            entryTasks.every((other) => t.id === other.id || !t.dependencies.includes(other.id))
          );
        }),
        { numRuns: 100 }
      );
