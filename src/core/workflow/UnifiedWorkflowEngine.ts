...
export class UnifiedWorkflowEngine extends BrowserEventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    const baseUrl =
      ...
      localStorage.getItem("claude_api_base_url") ||
   ...
   */
  private async generateWorkflowFromSpec(
    requirement: string,
      ...
      for (const taskSpec of phase.tasks) {
        ...
        const task: Task = {
          ...
          estimatedComplexity: Math.min(
            5,
            Math.max(1, Math.ceil((taskSpec.estimatedHours || 2) / 2))
          ) as 1 | 2 | 3 | 4 | 5,
          requiredSkills: [],
          requiredTools: [],
          status: "pending",
   ...
   * 计算关键路径
   */
  private calculateCriticalPathFromTasks(
    tasks: Task[],
    _entryPoints: string[],
    _exitPoints: string[]
  ): string[] {
  ...
  getProgress(): WorkflowProgress {
    ...
    const currentTask = this.workflow?.tasks.find((t) => this.inProgressTaskIds.has(t.id)) || null;
  ...
  private setupAgentSwarmEvents(): void {
    ...
    this.agentSwarmManager.on("task:failed", (event) => {
      ...
      this.addLog(
        "error",
        `任务失败: ${task.description} - ${event.error?.message}`,
        task.id,
        event.agent?.id
      );
