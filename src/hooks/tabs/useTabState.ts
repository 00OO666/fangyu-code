...
export function useTabState() {
   ...
   */
  const resolveTabTheme = useCallback((session?: Session, projectPath?: string): ThemeName => {
  ...
  const updateTabTheme = useCallback((tabId: string, themeName: ThemeName) => {
    setTabs((prev) =>
      prev.map((tab) => (tab.id === tabId ? { ...tab, themeName, lastActiveAt: Date.now() } : tab))
    );
  }, []);
    ...
    (
      ...
      setTabs((prev) =>
        prev.map((tab) => {
          ...
          logger.debug("useTabState", "[useTabState] Updating tab session:", {
            tabId,
            sessionInfo,
          });
    ...
    async (
      ...
      try {
        ...
        try {
          ...
          logger.warn(
            "useTabState",
            "[useTabState] Title generation failed, using fallback:",
            titleError
          );
        ...
        if (!result.success) {
          logger.error(
            "useTabState",
            "[useTabState] Failed to create smart project:",
            result.error
          );
          setTabs((prev) =>
            prev.map((t) =>
        ...
        try {
          ...
          logger.error(
            "useTabState",
            "[useTabState] Failed to create CLAUDE.md (non-blocking);:",
            error
          );
        }
