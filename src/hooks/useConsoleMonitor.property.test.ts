...
const errorTypeArb = fc.constantFrom<"error" | "warn" | "info">(
    "error",
    "warn",
    "info"
);
        ...
        await fc.assert(
            fc.property(errorEntriesArb, (entries) => {
                ...
                const uniqueIds = new Set(
                    entries.map((e) => `${e.type}-${e.message.substring(0, 100)}`)
                );
        ...
        await fc.assert(
            fc.property(
                ...
                (entries) => {
                    ...
                    expect(limitedManager.getImmediateErrorCount()).toBeLessThanOrEqual(
                        maxErrors
                    );
