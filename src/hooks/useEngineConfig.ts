...
export interface UseEngineConfigReturn {
  ...
  // 验证
  validateProvider: (config: Partial<UnifiedProviderConfig>) => {
    valid: boolean;
    errors: string[];
    warnings: string[];
  };
...
export function useEngineConfig(): UseEngineConfigReturn {
  ...
  const handleAddProvider = useCallback(
    async (config: Omit<UnifiedProviderConfig, "id" | "createdAt" | "updatedAt" | "sortOrder">) => {
  ...
  const handleUpdateProvider = useCallback(
    async (id: string, updates: Partial<UnifiedProviderConfig>) => {
  ...
  const handleTestConnection = useCallback(
    async (config: Partial<UnifiedProviderConfig>): Promise<ConnectionTestResult> => {
  ...
  const handleImportConfig = useCallback(
    async (data: ExportedConfig, mode: "merge" | "replace") => {
