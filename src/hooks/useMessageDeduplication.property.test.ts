        ...
        if (id) {
            ...
            if (existingMsg && existingMsg.message?.content && msg.message?.content) {
                ...
                const existingContent = Array.isArray(existingMsg.message.content)
                    ...
                    : [];
                const newContent = Array.isArray(msg.message.content)
                    ? msg.message.content
                    : [];

                const existingThinking = existingContent.filter(
                    (item: any) => item.type === "thinking"
                );
                const newThinking = newContent.filter(
                    (item: any) => item.type === "thinking"
                );
    ...
    it("should accurately count duplicates", async () => {
        await fc.assert(
            fc.property(messagesWithDuplicatesArb, (messages) => {
                ...
                expect(result.duplicateCount).toBe(
                    result.originalCount - result.deduplicatedCount
                );

                // Verify duplicate rate calculation
                const expectedRate =
                    result.originalCount > 0
                        ? result.duplicateCount / result.originalCount
                        : 0;
