...
export function useMultiEngineSettings(): UseMultiEngineSettingsReturn {
  const [settings, setSettings] = useState<MultiEngineSettingsStore>(
    createDefaultMultiEngineSettings
  );
  ...
  const getEngineSettings = useCallback(
    (engine: EngineType): EngineSettings => {
  ...
  // 更新引擎权限
  const updateEnginePermissions = useCallback(
    (engine: EngineType, permissions: PermissionRules) => {
      setSettings((prev) => ({
        ...prev,
        engines: {
          ...prev.engines,
