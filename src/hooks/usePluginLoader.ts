...
function createPluginAPI(_pluginId: string, _permissions: PluginPermission[]): PluginAPI {
  return {
    ...
    window: {
      showInformationMessage: async (message) => { console.log('[Info]', message); return undefined; },
      showWarningMessage: async (message) => { console.warn('[Warn]', message); return undefined; },
      showErrorMessage: async (message) => { console.error('[Error]', message); return undefined; },
...
export function usePluginLoader(options: UsePluginLoaderOptions = {}) {
   ...
   * 解析插件清单
   */
  const parseManifest = useCallback(async (manifestPath: string): Promise<PluginManifest | null> => {
    try {
      // 模拟读取清单文件
   ...
   * 加载单个插件
   */
  const loadPlugin = useCallback(async (pluginPath: string): Promise<PluginLoadResult> => {
   ...
   * 激活插件
   */
  const activatePlugin = useCallback(async (pluginId: string): Promise<PluginActivationResult> => {
   ...
   * 启用插件
   */
  const enablePlugin = useCallback((pluginId: string): boolean => {
    const pluginState = plugins.get(pluginId);
   ...
   * 触发激活事件
   */
  const triggerActivationEvent = useCallback(async (event: ActivationEvent): Promise<void> => {
      ...
      for (let i = 0; i < pluginDirs.length; i++) {
        ...
        if (result.success) {
          ...
          if (config.autoActivate && result.plugin) {
            const hasImmediateActivation = result.plugin.manifest.activationEvents.includes('*') ||
  ...
  }, [activatePlugin, config.autoActivate, config.pluginsDir, loadPlugin, triggerActivationEvent, workspacePath]);

  /**
   * 重新加载插件
   */
  const reloadPlugin = useCallback(async (pluginId: string): Promise<PluginLoadResult> => {
    const pluginState = plugins.get(pluginId);
   ...
   * 获取插件
   */
  const getPlugin = useCallback((pluginId: string): PluginState | undefined => {
