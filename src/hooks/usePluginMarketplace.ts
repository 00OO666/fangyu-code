...
const RECOMMENDED_PLUGINS = ['fangyu.ai-assistant', 'community.react-snippets', 'themes.one-dark-pro'];
...
export function usePluginMarketplace(options: UsePluginMarketplaceOptions = {}) {
  ...
  const searchPlugins = useCallback(async (
    searchOptions: PluginSearchOptions
    ...
    try {
      ...
      if (searchOptions.category) {
        results = results.filter(p =>
          p.categories.includes(searchOptions.category!)
        );
      }
   ...
   * 获取插件详情
   */
  const getPluginDetails = useCallback(async (
    pluginId: string
  ): Promise<PluginMarketplaceInfo | null> => {
  ...
  const getFeaturedPlugins = useCallback(async (): Promise<PluginMarketplaceInfo[]> => {
    ...
    try {
      ...
      // 获取推荐插件
      const featuredPlugins = MOCK_PLUGINS.filter(p =>
        RECOMMENDED_PLUGINS.includes(p.id)
      );
   ...
   * 获取分类下的插件
   */
  const getPluginsByCategory = useCallback(async (
    category: PluginCategory
  ): Promise<PluginMarketplaceInfo[]> => {
  ...
  const installPlugin = useCallback(async (
    pluginId: string,
    _installOptions?: Partial<PluginInstallOptions>
  ): Promise<boolean> => {
    ...
    // 更新安装进度
    const updateProgress = (status: InstallProgress['status'], progress: number, error?: string) => {
   ...
   * 获取安装进度
   */
  const getInstallProgress = useCallback((pluginId: string): InstallProgress | undefined => {
   ...
   * 检查插件是否已安装
   */
  const isInstalled = useCallback((pluginId: string): boolean => {
