...
} from "@/lib/translationMiddleware";
...
/**
 * 检测是否为 Claude Code CLI 的 thinking blocks 错误
 * 这是 Claude Code CLI 2.0.76 的已知 bug，主要在以下场景触发：
 * 1. 自动会话压缩（auto-compaction）
 * 2. 恢复包含 Opus 4.5 extended thinking 的会话
 * 3. 多轮对话使用 thinking 功能时
 *
 * 错误特征：
 * - API Error 400
 * - type: "invalid_request_error"
 * - message 包含 "thinking" 和 "blocks" 关键词
 * - message 包含 "cannot be modified" 或 "must remain"
 */
const isThinkingBlocksError = (error: unknown): boolean => {
  ...
  const hasBlocksKeyword = lowerError.includes("block");
  const hasModificationError =
    lowerError.includes("cannot be modified") ||
    lowerError.includes("must remain") ||
    lowerError.includes("redacted_thinking");
  const is400Error = lowerError.includes("400") || lowerError.includes("invalid_request");
...
export function usePromptExecution(config: UsePromptExecutionConfig): UsePromptExecutionReturn {
  ...
  const updateCodexRateLimits = useCallback(
    (incoming?: CodexRateLimits | null) => {
  ...
  const refreshCodexRateLimitsFromHistory = useCallback(async () => {
    ...
    const sessionId =
      codexThreadIdRef.current || extractedSessionInfo?.sessionId || effectiveSession?.id;
    if (!sessionId) {
      return;
    }
    ...
    try {
      ...
      logger.warn(
        "usePromptExecution",
        "[usePromptExecution] Failed to refresh Codex rate limits:",
        err
      );
    }
  }, [
  ...
  const handleSendPrompt = useCallback(
    async (
      ...
      try {
        ...
        logger.info(
          "usePromptExecution",
          `[usePromptExecution] 🔍 DIAGNOSTIC - Session State at prompt entry:`,
          {
            isFirstPrompt,
        ...
        if (isFirstPrompt && effectiveSession) {
          logger.warn(
            "usePromptExecution",
            `[usePromptExecution] ⚠️ State inconsistency detected: isFirstPrompt=true but effectiveSession exists. Auto-correcting to isFirstPrompt=false.`
          );
        ...
        if (!isFirstPrompt && !effectiveSession) {
          logger.warn(
            "usePromptExecution",
            `[usePromptExecution] ⚠️ State inconsistency detected: isFirstPrompt=false but effectiveSession is null. Auto-correcting to isFirstPrompt=true.`
          );
        ...
        if (effectiveSession && isUserInitiated) {
          if (executionEngine === "codex") {
            ...
            recordPromptPromise = api
              .recordCodexPromptSent(effectiveSession.id, projectPath, prompt)
              .then((index) => {
              ...
              .catch((err) => {
                logger.error(
                  "usePromptExecution",
                  "[Prompt Revert] [ERROR] Failed to record Codex prompt:",
                  err
                );
                return -1;
              });
            ...
            recordPromptPromise = api
              ...
              .then((index) => {
                ...
                logger.debug(
                  "usePromptExecution",
                  "[Prompt Revert] ✅ Background Git record completed, index:",
                  index
                );
                return index;
              })
              .catch((err) => {
                logger.error(
                  "usePromptExecution",
                  "[Prompt Revert] [ERROR] Failed to record prompt:",
                  err
                );
                return -1;
              });
        ...
        if (isAutoContinue) {
          ...
          logger.debug(
            "usePromptExecution",
            "[usePromptExecution] 🔄 Auto-continue detected, sending:",
            processedPrompt
          );
        }
        ...
        if (!isListeningRef.current && isActive) {
          ...
          if (executionEngine === "codex") {
            ...
            const processCodexOutput = async (payload: string) => {
              ...
              if (tabIdRef.current !== codexRequestTabId) {
                logger.debug(
                  "usePromptExecution",
                  "[usePromptExecution/Codex] ⚠️ tabId 已变化，忽略旧请求的消息"
                );
                return;
              }
                  ...
                  if (
                    ...
                    pendingPromptRecordingPromise = api
                      ...
                      .catch((err) => {
                        logger.warn(
                          "usePromptExecution",
                          "[usePromptExecution] Failed to record Codex prompt after init:",
                          err
                        );
                      });
            ...
            const processCodexComplete = async () => {
              ...
              if (window.__codexPendingPrompt) {
                ...
                try {
                  ...
                  logger.warn(
                    "usePromptExecution",
                    "[usePromptExecution] Failed to record Codex prompt completion:",
                    err
                  );
                }
            ...
            // 🔒 CRITICAL FIX: 支持 tab_id 过滤，解决多窗口并发时的消息串扰
            const codexSessionInitUnlisten = await listen<{
              type: string;
              session_id: string;
              tab_id?: string | null;
            }>("codex-session-init", async (evt) => {
            ...
            const codexOutputUnlisten = await listen<ClaudeGlobalEventPayload<string>>(
              "codex-output",
              (evt) => {
                ...
                // 只在会话ID未知的早期阶段处理
                processCodexOutput(messagePayload);
              }
            );

            // Listen for Codex errors
            // 🔒 CRITICAL FIX: 全局事件现在格式为 { tab_id: string | null, payload: string }
            const codexErrorUnlisten = await listen<ClaudeGlobalEventPayload<string>>(
              "codex-error",
              async (evt) => {
                // 🔧 FIX: Only process if this tab has an active session
                if (!hasActiveSessionRef.current) return;
                ...
                const parsed = parseCodexErrorPayload(errorPayload);

                // 🔒 Session Isolation：如果已确定会话 ID，则忽略其他会话的错误
                if (
                  parsed.sessionId &&
                  currentCodexSessionId &&
                  parsed.sessionId !== currentCodexSessionId
                ) {
                  return;
                }
                ...
                await processCodexError(errorPayload);
              }
            );
            ...
            const codexCompleteUnlisten = await listen<ClaudeGlobalEventPayload<boolean>>(
              "codex-complete",
              async (evt) => {
                // 🔧 CRITICAL FIX: 只在尚未收到会话ID时处理全局事件
                if (!hasActiveSessionRef.current) return;
            ...
            unlistenRefs.current = [
              codexSessionInitUnlisten,
              codexOutputUnlisten,
              codexErrorUnlisten,
              codexCompleteUnlisten,
            ];
          } else if (executionEngine === "gemini") {
            // ====================================================================
            ...
            // Note: Using 'unknown' for data parameter as it comes from external source (backend)
            const convertGeminiToClaudeMessage = (data: unknown): ClaudeStreamMessage | null => {
              try {
                ...
                if (msg.type === "assistant" || msg.type === "user") {
                  ...
                  if (msg.type === "user" && message?.content) {
                ...
                if (msg.type === "result") {
                  return {
                    ...
                    engine: "gemini" as const,
                    model: msg.model,
                    geminiMetadata: msg.geminiMetadata,
                  };
                }
                ...
                return {
                  type: "system",
                  subtype: "raw",
                  message: { content: [{ type: "text", text: JSON.stringify(msg) }] },
                  engine: "gemini" as const,
                };
              } catch (err) {
                logger.error(
                  "usePromptExecution",
                  "[usePromptExecution] Failed to convert Gemini message:",
                  err
                );
                return null;
              }
            ...
            const processGeminiOutput = (payload: string) => {
              ...
              if (tabIdRef.current !== geminiRequestTabId) {
                logger.debug(
                  "usePromptExecution",
                  "[usePromptExecution/Gemini] ⚠️ tabId 已变化，忽略旧请求的消息"
                );
                return;
              }
              ...
              try {
                      ...
                      if (Array.isArray(lastContent) && Array.isArray(newContent)) {
                        ...
                        for (const newItem of newContent) {
                          if (newItem.type === "text") {
                            ...
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            const lastTextIdx = updatedContent.findIndex(
                              (c: any) => c.type === "text"
                            );
                ...
                logger.error(
                  "usePromptExecution",
                  "[usePromptExecution] Failed to process Gemini output:",
                  err,
                  payload
                );
              }
            ...
            const processGeminiComplete = async () => {
              ...
              if (window.__geminiPendingPrompt) {
                ...
                try {
                  ...
                  logger.warn(
                    "usePromptExecution",
                    "[usePromptExecution] Failed to record Gemini prompt completion:",
                    err
                  );
                }
                // Clear the pending prompt
            ...
            const geminiSessionInitUnlisten = await listen<any>(
              "gemini-session-init",
              async (evt) => {
                if (!hasActiveSessionRef.current) return;
            ...
            const geminiCliSessionIdUnlisten = await listen<{
              backend_session_id: string;
              cli_session_id: string;
            }>("gemini-cli-session-id", async (evt) => {
              if (!hasActiveSessionRef.current) return;
              ...
              if (
                isUserInitiated &&
                geminiPendingInfo &&
                geminiPendingInfo.promptIndex === undefined
              ) {
                pendingGeminiPromptRecordingPromise = api
                  .recordGeminiPromptSent(
                    realCliSessionId,
                    projectPath,
                    geminiPendingInfo.promptText
                  )
                  .then((idx) => {
                  ...
                  .catch((err) => {
                    logger.warn(
                      "usePromptExecution",
                      "[Gemini Revert] Failed to record prompt with real CLI session ID:",
                      err
                    );
                  });
              }
            ...
            unlistenRefs.current = [
              geminiSessionInitUnlisten,
              geminiCliSessionIdUnlisten,
              geminiOutputUnlisten,
              geminiErrorUnlisten,
              geminiCompleteUnlisten,
            ];
          } else {
            // --------------------------------------------------------------------
                      ...
                      if (msgContent) {
                        if (typeof msgContent === "string") {
                          ...
                          const textContent = msgContent
                            ...
                            .join("");
                          isOurMessage =
                            textContent.includes(prompt) || prompt.includes(textContent);
                        }
                      }
            ...
            async function handleStreamMessage(
              ...
              try {
                ...
                if (tabIdRef.current !== currentRequestTabId) {
                  logger.debug(
                    "usePromptExecution",
                    "[usePromptExecution] ⚠️ tabId 已变化，忽略旧请求的消息"
                  );
                  return;
                }
                ...
                if (sessionId && projectId) {
                  api
                    ...
                    .catch((err) => {
                      logger.error(
                        "usePromptExecution",
                        "[Prompt Revert] Failed to mark completed:",
                        err
                      );
                    });
                } else {
                  logger.warn(
                    "usePromptExecution",
                    "Cannot mark completed: missing sessionId or projectId"
                  );
                }
              }
                ...
                try {
                  ...
                  if (msg.type === "system" && msg.subtype === "init" && msg.session_id) {
                    if (!currentSessionId || currentSessionId !== msg.session_id) {
                      ...
                      logger.info(
                        "usePromptExecution",
                        `[usePromptExecution] ✅ Session initialized: sessionId=${msg.session_id}, isFirstPrompt set to false`
                      );
            ...
            const genericErrorUnlisten = await listen<ClaudeGlobalEventPayload<string>>(
              ...
              (evt) => {
                ...
                if (isThinkingBlocksError(errorPayload)) {
                  logger.warn(
                    "usePromptExecution",
                    "Detected thinking blocks error (Claude Code CLI bug)"
                  );
                  const friendlyMessage =
                    "⚠️ 会话历史包含无法处理的 thinking 块（Claude Code CLI 已知问题）\n\n" +
                    "解决方案：\n" +
                    '1. 点击右上角 "+" 开启新会话\n' +
                    "2. 或运行: claude settings set autoCompact false\n" +
                    "3. 等待 Claude Code CLI 更新修复";
          ...
          if (!isSlashCommandInput) {
            try {
              ...
              const isEnabled =
                translationMiddleware.isEnabledSync() || (await translationMiddleware.isEnabled());
              if (isEnabled) {
          ...
          if (isSlashCmd) {
            ...
            setMessages((prev) => {
              logger.debug(
                "usePromptExecution",
                "setMessages called, current messages count:",
                prev.length
              );
        ...
        if (executionEngine === "codex") {
          ...
          const pendingIndex =
            recordedPromptIndex >= 0 ? recordedPromptIndex : codexPendingInfo?.promptIndex;
          ...
          const pendingIndex =
            recordedPromptIndex >= 0 ? recordedPromptIndex : geminiPendingInfo?.promptIndex;
          ...
          logger.debug(
            "usePromptExecution",
            `[usePromptExecution] ⏱️ Pre-API processing took ${(performance.now() - perfStart).toFixed(0)}ms`
          );

          // 🔧 DEBUG: 记录会话状态，帮助诊断上下文丢失问题
          logger.info(
            "usePromptExecution",
            `[usePromptExecution] 📊 Session State: isFirstPrompt=${isFirstPrompt}, hasEffectiveSession=${!!effectiveSession}, sessionId=${effectiveSession?.id || "none"}`
          );
          ...
          if (effectiveSession && !isFirstPrompt) {
            ...
            logger.info(
              "usePromptExecution",
              `[usePromptExecution] ▶️ RESUMING session: ${effectiveSession.id}`
            );
            try {
              ...
              if (isSessionNotFoundError(resumeError)) {
                logger.warn(
                  "usePromptExecution",
                  "Session file not found, falling back to NEW session"
                );
                ...
                const warningMessage: ClaudeStreamMessage = {
                  ...
                  message: {
                    content: [
                      {
                        type: "text",
                        text: "⚠️ 原会话包含无法处理的 thinking 块，已自动开启新会话继续。\n（这是 Claude Code CLI 的已知问题，建议运行: claude settings set autoCompact false）",
                      },
                    ],
                  },
                  timestamp: new Date().toISOString(),
                ...
                logger.warn(
                  "usePromptExecution",
                  "[usePromptExecution] Falling back to continue mode"
                );
                try {
            ...
            logger.info(
              "usePromptExecution",
              `[usePromptExecution] 🆕 STARTING NEW session (isFirstPrompt=${isFirstPrompt}, effectiveSession=${!!effectiveSession})`
            );
            setIsFirstPrompt(false);
            await api.executeClaudeCode(
              projectPath,
