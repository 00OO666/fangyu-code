...
const isThinkingBlocksError = (error: any): boolean => {
  ...
  const hasModificationError = lowerError.includes('cannot be modified') ||
    ...
    try {
      ...
      if (!isListeningRef.current && isActive) {
        ...
        if (executionEngine === 'codex') {
          ...
          const codexSessionInitUnlisten = await listen<{ type: string; session_id: string; tab_id?: string | null }>('codex-session-init', async (evt) => {
          ...
          const codexOutputUnlisten = await listen<ClaudeGlobalEventPayload<string>>('codex-output', (evt) => {
            // 🔧 CRITICAL FIX: 只在尚未收到会话ID时处理全局事件
            if (!hasActiveSessionRef.current) return;

            // 🔒 CRITICAL FIX: 使用 tab_id 过滤消息
            const { tabId: eventTabId, payload: messagePayload } = normalizeClaudeGlobalPayload(evt.payload);
            if (eventTabId && eventTabId !== tabIdRef.current) {
              // 消息来自不同标签页，忽略
              return;
            }
          ...
          // 🔒 CRITICAL FIX: 全局事件现在格式为 { tab_id: string | null, payload: string }
          const codexErrorUnlisten = await listen<ClaudeGlobalEventPayload<string>>('codex-error', async (evt) => {
            // 🔧 FIX: Only process if this tab has an active session
            if (!hasActiveSessionRef.current) return;
          ...
          const codexCompleteUnlisten = await listen<ClaudeGlobalEventPayload<boolean>>('codex-complete', async (evt) => {
            // 🔧 CRITICAL FIX: 只在尚未收到会话ID时处理全局事件
            if (!hasActiveSessionRef.current) return;
        ...
        const genericErrorUnlisten = await listen<ClaudeGlobalEventPayload<string>>('claude-error', (evt) => {
          ...
          if (isThinkingBlocksError(errorPayload)) {
            console.warn('[usePromptExecution] Detected thinking blocks error (Claude Code CLI bug)');
            const friendlyMessage = '⚠️ 会话历史包含无法处理的 thinking 块（Claude Code CLI 已知问题）\n\n' +
              '解决方案：\n' +
              '1. 点击右上角 "+" 开启新会话\n' +
        ...
        if (isSlashCmd) {
          ...
          setMessages(prev => {
            console.log('[usePromptExecution] setMessages called, current messages count:', prev.length);
      ...
      if (executionEngine === 'codex') {
        ...
        if (!siliconflowConfig.apiKey) {
          throw new Error('SiliconFlow API Key 未配置。请在聊天界面右下角点击 SiliconFlow 按钮配置。');
        }
        ...
        if (effectiveSession && !isFirstPrompt) {
          ...
          try {
            ...
            if (isThinkingBlocksError(resumeError)) {
              console.warn('[usePromptExecution] Detected thinking blocks error, falling back to NEW session');
              ...
              try {
                ...
                if (isThinkingBlocksError(continueError)) {
                  console.warn('[usePromptExecution] Continue also failed with thinking blocks error, falling back to NEW session');
