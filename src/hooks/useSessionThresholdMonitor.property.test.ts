...
class ThresholdMonitorManager {
  ...
  checkThreshold(currentTokens: number): {
    ...
    if (percentage > 1.0 && this.currentTime - this.lastExceedWarningTime > WARNING_INTERVAL) {
  ...
  it("should accurately track threshold state regardless of rate limiting", () => {
    fc.assert(
      fc.property(tokenCountArb, configArb, (tokens, config) => {
        ...
        expect(status.isWarning).toBe(expectedPercentage >= config.warningThreshold);

        // Verify critical threshold detection
        expect(status.isCritical).toBe(expectedPercentage >= config.criticalThreshold);
  ...
  it("should have warning count bounded by time / interval", () => {
    fc.assert(
      fc.property(tokenUpdateSequenceArb, (updates) => {
        ...
        const maxPossibleWarnings = Math.floor(totalTime / WARNING_INTERVAL) + 1;
        expect(manager.getWarningCount()).toBeLessThanOrEqual(maxPossibleWarnings);
