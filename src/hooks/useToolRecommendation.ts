...
const KEYWORD_TOOL_MAPPING: Array<{
  ...
  // GitHub 相关
  {
    keywords: ['github', 'git', '仓库', 'repository', 'pr', 'pull request', 'issue', 'fork', 'clone', 'commit'],
  ...
  // 文件系统相关
  {
    keywords: ['文件', 'file', '目录', 'directory', '读取', '写入', 'read', 'write', '创建文件', 'create file'],
  ...
  // 浏览器自动化相关
  {
    keywords: ['截图', 'screenshot', '爬虫', 'crawler', 'scrape', '浏览器', 'browser', '自动化', 'puppeteer', '页面'],
  ...
  // 复杂推理相关
  {
    keywords: ['分析', 'analyze', '推理', 'reasoning', '复杂', 'complex', '步骤', 'step by step', '思考', 'think'],
...
export function useToolRecommendation() {
   ...
   * 分析文本内容，生成工具推荐
   */
  const analyzeContent = useCallback((content: string, enabledTools: Set<string> = new Set()) => {
