...
const taskArbitrary = fc.record({
  ...
  status: fc.constantFrom(
    ...
    "failed",
    "cancelled"
  ) as fc.Arbitrary<"pending" | "in_progress" | "completed" | "failed" | "cancelled">,
...
describe("Property 14: Hook Cleanup", () => {
  ...
  it("off removes specific handler", () => {
    fc.assert(
      fc.property(
        fc.constantFrom("workflow:started", "workflow:completed", "task:started", "task:completed"),
  ...
  it("multiple subscriptions can be cleaned up independently", () => {
    fc.assert(
      fc.property(fc.integer({ min: 2, max: 10 }), (subscriptionCount) => {
        const emitter = new MockEventEmitter();
