...
import type { WorkflowDAG, WorkflowLog, Agent, Task, WorkflowEvent } from "../core/types/workflow";
...
export function useUnifiedWorkflow(
  ...
  const config = useMemo<Partial<UnifiedWorkflowConfig>>(
    () => ({
      api: {
        ...
        baseUrl:
          apiBaseUrl || localStorage.getItem("claude_api_base_url") || "https://hiapi.online/v1",
        model,
      },
      agents: {
        maxAgents,
  ...
  const generateWorkflow = useCallback(
    async (requirement: string, genOptions?: GenerateOptions): Promise<WorkflowDAG> => {
      if (!engineRef.current) {
        await initialize();
      }
  ...
  const subscribe = useCallback(
    (eventType: string, handler: (event: WorkflowEvent) => void): (() => void) => {
      if (!engineRef.current) {
        logger.warn(
          "useUnifiedWorkflow",
          "[useUnifiedWorkflow] Engine not initialized, subscription will be ignored"
        );
        return () => {};
      }
