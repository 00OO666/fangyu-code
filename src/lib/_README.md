...
```typescript
export async function translateMessage(message: Message, targetLang: string): Promise<Message>;

export function extractTranslatableContent(content: string): string[];

export function mergeTranslation(original: string, translation: string): string; // 双语格式
```
