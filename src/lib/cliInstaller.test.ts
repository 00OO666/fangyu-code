...
describe("installCli", () => {
  ...
  it("should install globally and verify successfully", async () => {
    ...
    expect(executeCommandMock.mock.calls[0]?.[0]).toBe("npm install -g @anthropic-ai/claude-code");
  ...
  it("should fallback to local install when global fails", async () => {
    ...
    expect(executeCommandMock.mock.calls[1]?.[0]).toBe("npm install @anthropic-ai/claude-code");
  });
