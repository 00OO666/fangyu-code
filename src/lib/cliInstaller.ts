...
const buildInstallCommand = (packageName: string, scope: CliInstallScope) => {
  return scope === "global" ? `npm install -g ${packageName}` : `npm install ${packageName}`;
};

const buildVerifyCommands = (config: CliConfig, scope: CliInstallScope) => {
  if (scope === "local") {
    return [`${config.command} ${config.versionArg}`, `npx ${config.command} ${config.versionArg}`];
  }
...
export async function installCli(
  ...
  try {
    ...
    if (!localResult.success) {
      ...
      const verification = await verifyCli(config, "local", verifyTimeoutMs, options.cwd, report);

      report("complete", `${config.name} 安装成功（本地）。建议稍后使用管理员权限全局安装。`);
