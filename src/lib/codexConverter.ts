...
function parseCodexRateLimitEntry(
  ...
  const windowMinutesValue =
    raw.window_minutes ?? raw.windowMinutes ?? raw.window_min ?? raw.window ?? raw.windowMinutes;
  ...
  return {
    ...
    windowMinutes:
      Number.isFinite(windowMinutes) && windowMinutes > 0 ? windowMinutes : fallbackWindowMinutes,
...
export function parseCodexRateLimits(raw: any, updatedAt: string): CodexRateLimits | null {
  ...
  const primaryCandidate =
    raw.primary ?? raw.primary_limit ?? raw.five_hour ?? raw.fiveHour ?? raw["5h"];
  const secondaryCandidate =
    raw.secondary ?? raw.secondary_limit ?? raw.weekly ?? raw.week ?? raw["1w"] ?? raw["7d"];
...
export class CodexEventConverter {
  ...
  convertEventObject(event: CodexEvent): ClaudeStreamMessage | null {
    switch (event.type) {
      ...
      case "thread_token_usage_updated": {
        // 参考: https://hexdocs.pm/codex_sdk/05-api-reference.html // 累计 token 使用量更新事件 - 这是关键的累计追踪事件
        const rateLimits = extractCodexRateLimitsFromEvent(event);
        if (rateLimits) {
          this.latestRateLimits = rateLimits;
        }
  ...
  private convertResponseItem(
    ...
    if (payload.role === "user" && payload.content) {
      const isEnvContext = payload.content.some(
        (c: any) =>
          c.type === "input_text" &&
          c.text &&
  ...
  private convertFileChange(
    ...
    const toolName =
      item.change_type === "create" ? "write" : item.change_type === "delete" ? "bash" : "edit";
  ...
  private buildFileChangeSummary(item: CodexFileChangeItem): string {
    ...
    const snippet =
      snippetSource.length > 800 ? `${snippetSource.slice(0, 800)}\n...[truncated]` : snippetSource;
    return `${header}\n${snippet}`;
  }
  ...
  private convertMcpToolCall(
    ...
    const toolName = item.server
      ? `mcp__${item.server}__${item.tool}`
      : item.tool || item.tool_name;
