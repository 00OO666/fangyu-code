...
export class ErrorHandler {
  ...
  private handleGenericError(error: Error, context: ErrorContext): ClaudeError {
    ...
    // CLI not found
    if (
      lowerMessage.includes("claude cli not found") ||
        ...
        actions: [
          {
            ...
            action: () => {
              window.open("https://docs.anthropic.com/claude/docs/claude-cli", "_blank");
            },
            primary: true,
          },
          {
    ...
    if (
      lowerMessage.includes("spawning") ||
    ...
    if (
      ...
      return new ClaudeError({
        code: lowerMessage.includes("claude")
          ? "CLAUDE_NETWORK_ERROR"
          : "NETWORK_CONNECTION_FAILED",
        type: lowerMessage.includes("claude")
    ...
    if (
      ...
      return new ClaudeError({
        ...
        type: lowerMessage.includes("claude")
          ? ErrorType.CLAUDE_CONFIG_ERROR
          : ErrorType.SDK_CONFIGURATION_ERROR,
        message: error.message,
  ...
  const wrappedFn = async (...args: Parameters<T>) => {
    try {
      if (options?.retryable && options?.retryHandler) {
        return await options.retryHandler.execute(
          () => fn(...args),
          (error) => {
            const claudeError = parseClaudeError(error);
            return claudeError.retryable;
  ...
  async execute<T>(
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const delay = Math.min(this.baseDelay * 2 ** attempt + Math.random() * 1000, this.maxDelay);
