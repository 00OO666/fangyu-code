...
export const MODEL_PRICING: Record<string, ModelPricing> = {
  ...
  "gpt-5.1-codex": {
    ...
    output: 10.0, // $10.00 / 1M output tokens
  ...
  "codex-mini-latest": {
    input: 1.5, // $1.50 / 1M input tokens (官方定价)
    output: 6.0, // $6.00 / 1M output tokens
    cacheWrite: 0, // OpenAI cache write 免费
  ...
  "gpt-5.2": {
    ...
    output: 14.0, // $14.00 / 1M output tokens
    cacheWrite: 0, // OpenAI cache write 免费
  ...
  "o4-mini": {
    input: 1.1, // $1.10 / 1M input tokens (Standard tier)
...
function getGeminiTieredPricing(model: string, promptTokens: number): ModelPricing {
  ...
  if (lower.includes("gemini-3-pro") || lower.includes("gemini_3_pro")) {
    return {
      input: isOver200k ? 4.0 : 2.0,
      output: isOver200k ? 18.0 : 12.0,
      cacheWrite: 0.0,
      cacheRead: isOver200k ? 0.4 : 0.2,
    };
  }
  ...
  if (lower.includes("2.5-pro") || lower.includes("2_5_pro")) {
    return {
      input: isOver200k ? 2.5 : 1.25,
      output: isOver200k ? 15.0 : 10.0,
      cacheWrite: 0.0,
 ...
 * @returns 格式化的字符串（如 "15.2k" 或 "1.5M"）
 */
