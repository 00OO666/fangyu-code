...
import { maskAPIKey, type APIKeyProvider } from "./secureStorage";
...
// 生成有效的 API 密钥
const validAPIKey = fc.stringOf(fc.constantFrom(...alphanumericWithSpecialChars.split("")), {
  minLength: 20,
  maxLength: 150,
});
  ...
  describe("Property 7: API 密钥安全存储", () => {
    describe("maskAPIKey 函数", () => {
      it("遮罩后的密钥应保留前后指定数量的字符", () => {
        fc.assert(
          fc.property(validAPIKey, fc.integer({ min: 1, max: 8 }), (key, visibleChars) => {
            // 只测试足够长的密钥
      ...
      it("短密钥应完全遮罩", () => {
        fc.assert(
          fc.property(
            fc.stringOf(fc.constantFrom(...alphanumericChars.split("")), {
              minLength: 1,
              maxLength: 8,
            }),
            (key) => {
      ...
      fc.assert(
        fc.property(
          ...
          (key1, key2) => {
            ...
            if (key1.slice(0, 4) === key2.slice(0, 4) && key1.slice(-4) === key2.slice(-4)) {
              return;
            }
