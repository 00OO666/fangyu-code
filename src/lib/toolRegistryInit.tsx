...
import { type ToolRenderer, type ToolRenderProps, toolRegistry } from "./toolRegistry";
  ...
  const tools: ToolRenderer[] = [
    ...
    // Summary - 会话总结展示
    {
      ...
      render: createToolAdapter(SummaryWidget, (props) => ({
        ...
        leafUuid:
          props.input?.leafUuid ?? props.input?.leaf_uuid ?? props.result?.content?.leafUuid,
    ...
    // System Initialized - 系统初始化信息
    {
      ...
      render: createToolAdapter(SystemInitializedWidget, (props) => ({
        sessionId:
          props.input?.sessionId ?? props.input?.session_id ?? props.result?.content?.sessionId,
