...
async function migrateFromLegacy(): Promise<ProviderStorage> {
  ...
  try {
    ...
    if (currentConfig.anthropic_base_url) {
      ...
      if (matchingProvider) {
        ...
        logger.debug(
          "configMigrator",
          "[ConfigMigrator] 找到当前激活的 Claude 代理商:",
  ...
  logger.debug(
    "configMigrator",
    `[ConfigMigrator] 从旧格式迁移了 ${providers.length} 个代理商配置`
  );
...
export async function migrate(): Promise<MigrationResult> {
  ...
  try {
    ...
    logger.debug(
      "configMigrator",
      `[ConfigMigrator] 迁移完成: ${migratedStorage.providers.length} 个代理商`
    );
