...
describe("EngineConfigService Property Tests", () => {
  ...
  describe("引擎切换", () => {
    it("切换引擎后当前引擎应正确更新", () => {
      fc.assert(
        fc.property(engineTypeArb, engineTypeArb, (initialEngine, newEngine) => {
          // 模拟引擎切换
  ...
  describe("表单验证", () => {
    ...
    it("无效 URL 应验证失败", () => {
      fc.assert(
        fc.property(fc.constantFrom("not-a-url", "ftp://invalid", ""), (url) => {
  ...
  describe("引擎配置隔离 (Property 3)", () => {
    ...
    // 模拟更新 Claude 配置的函数
    const updateClaudeConfig = (configs: EngineConfigs, newModel: string): EngineConfigs => {
      return {
        ...configs,
...
describe("Node.js 版本验证 (Property 5)", () => {
  ...
  describe("parseNodeVersion", () => {
    ...
    it("无效版本字符串应返回 null", () => {
      fc.assert(
        fc.property(
          fc.constantFrom("", "invalid", "node", "v", "18", "v18", "18.0", "abc.def.ghi"),
          (invalidVersion) => {
  ...
  describe("isNodeVersionValid", () => {
    it("版本 >= 18 应返回 true", () => {
      fc.assert(
        fc.property(fc.integer({ min: 18, max: 100 }), (majorVersion) => {
    ...
    it("版本 < 18 应返回 false", () => {
      fc.assert(
        fc.property(fc.integer({ min: 0, max: 17 }), (majorVersion) => {
