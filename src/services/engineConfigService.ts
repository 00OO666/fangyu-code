 ...
 * 验证代理商配置
 */
export function validateProviderConfig(config: Partial<UnifiedProviderConfig>): ValidationResult {
  const errors: string[] = [];
 ...
 */
function toTauriConfig(
  config: UnifiedProviderConfig,
  decryptedApiKey?: string,
  decryptedAuthToken?: string
): TauriProviderConfig {
  return {
    id: config.id,
    name: config.name,
 ...
 */
async function syncClaudeProviderToBackend(
  config: UnifiedProviderConfig,
  ...
  try {
    ...
    if (action === "add") {
      ...
      logger.debug(
        "engineConfigService",
        "[EngineConfigService] 已同步添加 Claude 代理商到后端:",
        config.name
      );
      ...
      logger.debug(
        "engineConfigService",
        "[EngineConfigService] 已同步更新 Claude 代理商到后端:",
        config.name
      );
      ...
      logger.debug(
        "engineConfigService",
        "[EngineConfigService] 已同步删除 Claude 代理商从后端:",
        config.name
      );
    }
  } catch (error) {
    logger.error(
      "engineConfigService",
      "[EngineConfigService] 同步 Claude 代理商到后端失败:",
      error
    );
    // 不抛出错误，允许前端继续工作
  }
 ...
 * 重新排序代理商
 */
export async function reorderProviders(engine: EngineType, orderedIds: string[]): Promise<void> {
  const storage = readStorage();
...
async function syncClaudeEnvVarsToBackend(envVars?: RuntimeConfig["claudeEnvVars"]): Promise<void> {
  ...
  try {
    ...
    envConfig.CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC =
 ...
 */
export async function applyModelToClaudeCode(providerId: string, modelId: string): Promise<void> {
  const provider = getProvider(providerId);
  if (!provider) {
  ...
  logger.debug(
    "engineConfigService",
    "[EngineConfigService] 已全局应用模型到 Claude Code:",
    modelId
  );
}
...
async function syncClaudeEnvVarsFromBackend(): Promise<void> {
  try {
    ...
    if (currentConfig.max_thinking_tokens) {
      mergedEnvVars.MAX_THINKING_TOKENS =
        parseInt(currentConfig.max_thinking_tokens, 10) || undefined;
    }
    if (currentConfig.claude_code_max_output_tokens) {
      mergedEnvVars.CLAUDE_CODE_MAX_OUTPUT_TOKENS =
        parseInt(currentConfig.claude_code_max_output_tokens, 10) || undefined;
    }

    // 布尔类型
    if (
      currentConfig.claude_code_disable_nonessential_traffic === "1" ||
      currentConfig.claude_code_disable_nonessential_traffic === "true"
    ) {
      mergedEnvVars.CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC = true;
    }
    if (
      currentConfig.claude_code_disable_telemetry === "1" ||
      currentConfig.claude_code_disable_telemetry === "true"
    ) {
      mergedEnvVars.CLAUDE_CODE_DISABLE_TELEMETRY = true;
    }
    if (
      currentConfig.claude_code_use_bedrock === "1" ||
    ...
    logger.warn(
      "engineConfigService",
      "[EngineConfigService] 从后端同步 Claude 环境变量失败:",
      error
    );
    // 不抛出错误，允许应用继续启动
  }
