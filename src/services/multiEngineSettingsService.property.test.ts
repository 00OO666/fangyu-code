...
/** 生成两个不同的引擎类型 */
const twoDistinctEnginesArb = fc.tuple(engineTypeArb, engineTypeArb).filter(([a, b]) => a !== b);
...
/** 生成环境变量键 */
const envKeyArb = fc.stringOf(fc.constantFrom("A", "B", "C", "D", "E", "_"), {
  minLength: 1,
  maxLength: 20,
});
...
describe("Multi-Engine Settings Service Property Tests", () => {
  ...
  describe("Property 1: Engine Configuration Independence", () => {
    it("updating one engine permissions does not affect other engines", () => {
      fc.assert(
        fc.property(twoDistinctEnginesArb, permissionsArb, ([engine1, engine2], newPermissions) => {
    ...
    it("updating one engine env does not affect other engines", () => {
      fc.assert(
        fc.property(twoDistinctEnginesArb, envArb, ([engine1, engine2], newEnv) => {
          // Get original settings for engine2
      ...
      fc.assert(
        fc.property(
          ...
          (claudeSettings, codexSettings, geminiSettings) => {
            ...
            expect(newService.getEngineSettings("claude").permissions).toEqual(
              claudeSettings.permissions
            );
            expect(newService.getEngineSettings("codex").permissions).toEqual(
              codexSettings.permissions
            );
            expect(newService.getEngineSettings("gemini").permissions).toEqual(
              geminiSettings.permissions
            );
          }
        ),
        { numRuns: 50 }
  ...
  describe("Property 7: Settings Migration Preservation", () => {
    ...
    it("migration sets claude-code as default engine", () => {
      fc.assert(
        fc.property(legacySettingsArb, (legacy) => {
          const migrated = service.migrateFromLegacy(legacy);
    ...
    it("migration preserves general settings", () => {
      fc.assert(
        fc.property(fc.constantFrom("Chinese", "English"), fc.boolean(), (language, verbose) => {
  ...
  describe("Property 9: Engine Default Initialization", () => {
    ...
    it("claude-code has anthropic-specific default env vars", () => {
      ...
      expect("ANTHROPIC_API_KEY" in settings.env || "ANTHROPIC_BASE_URL" in settings.env).toBe(
        true
      );
    });
