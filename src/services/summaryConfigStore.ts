  ...
  async loadConfig(): Promise<SummaryAPIConfig> {
    try {
      ...
      if (parsed.version !== SUMMARY_CONFIG_VERSION) {
        logger.warn(
          "summaryConfigStore",
          "[SummaryConfigStore] Config version mismatch, resetting to defaults..."
        );
      ...
      if (!validation.valid) {
        logger.error(
          "summaryConfigStore",
          "[SummaryConfigStore] Invalid config, resetting to defaults:",
          validation.errors
        );
        this.config = { ...DEFAULT_SUMMARY_CONFIG };
        await this.saveConfig(this.config);
      ...
      logger.error(
        "summaryConfigStore",
        "[SummaryConfigStore] Failed to load config, resetting to defaults:",
        error
      );
      this.config = { ...DEFAULT_SUMMARY_CONFIG };
      return this.config;
