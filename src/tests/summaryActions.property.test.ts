...
describe("Property 7: Clipboard Copy Integrity", () => {
  ...
  it("should preserve text content exactly after copy", async () => {
    await fc.assert(
      fc.asyncProperty(fc.string({ minLength: 0, maxLength: 1000 }), async (text) => {
        clipboard.reset();
  ...
  it("should preserve unicode characters", async () => {
    await fc.assert(
      fc.asyncProperty(fc.unicodeString({ minLength: 0, maxLength: 500 }), async (text) => {
        clipboard.reset();
        await copyToClipboard(text);
        const result = await readFromClipboard();
        expect(result).toBe(text);
  ...
  it("should return true on successful copy", async () => {
    await fc.assert(
      fc.asyncProperty(fc.string({ minLength: 0, maxLength: 100 }), async (text) => {
        clipboard.reset();
