...
/** 生成有效的 API 端点 */
const apiEndpointArb: fc.Arbitrary<string | undefined> = fc.option(
    fc.webUrl(),
    { nil: undefined }
);
            ...
            await fc.assert(
                fc.asyncProperty(validConfigArb, async (config) => {
                    ...
                    if (config.customParams) {
                        ...
                        expect(loaded.customParams?.includeCodeSnippets).toBe(config.customParams.includeCodeSnippets);
                    }
                }),
                { numRuns: 100 }
            );
