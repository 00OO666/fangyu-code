...
import { SummaryConfigStore, createSummaryConfigStore } from "@/services/summaryConfigStore";
import {
...
describe("Summary Generator Property Tests", () => {
  ...
  describe("Property 1: Config Isolation", () => {
    ...
    it("should maintain separate storage keys for summary and main configs", async () => {
      await fc.assert(
        fc.asyncProperty(validConfigArb, mainChatConfigArb, async (summaryConfig, mainConfig) => {
          const mainChatKey = "fangyu-main-chat-config";
  ...
  describe("Property 8: API Fallback Behavior", () => {
    ...
    it("should reset to defaults when config is corrupted", async () => {
      await fc.assert(
        fc.asyncProperty(fc.string({ minLength: 1, maxLength: 100 }), async (corruptedData) => {
    ...
    it("should use provided config over stored config", async () => {
      await fc.assert(
        fc.asyncProperty(validConfigArb, validConfigArb, async (storedConfig, providedConfig) => {
          // Store one config
